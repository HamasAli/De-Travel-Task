...
import { config, TourPackage, OFFICIAL_CANCELLATION_RULES, OFFICIAL_CANCELLATION_NOTE } from "@/config";
...
export default function SinglePackagePage({ params }: { params: { slug: string } }) {
  ...
  const [activeDetailsTab, setActiveDetailsTab] = useState<"inclusions" | "dining" | "guidelines" | "policies">("inclusions");
...
Hello De Travel Task! I would like to inquire about this tour, check seat availability, and get the official package quotation.`;
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8">
        ...
        <div className="mb-6 max-w-4xl">
          <div className="flex flex-wrap items-center gap-2.5 mb-3">
            ...
            <div 
              className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-semibold bg-emerald-50 text-emerald-800 border border-emerald-200/70"
              title="Govt. Registered Pvt Ltd"
            >
              <ShieldCheck className="w-3.5 h-3.5 text-emerald-600" />
              <span>Govt. Reg: {config.registrationId}</span>
            </div>
        ...
        <div className="sticky top-16 z-20 -mx-4 sm:-mx-6 lg:-mx-8 px-4 sm:px-6 lg:px-8 py-2.5 bg-white/95 backdrop-blur-md border-y border-slate-200 mb-10 overflow-x-auto no-scrollbar shadow-2xs">
          <div className="max-w-7xl mx-auto flex items-center gap-1 sm:gap-2 text-xs font-semibold whitespace-nowrap text-slate-600">
            ...
            <a href="#cancellation" className="px-3.5 py-1.5 rounded-lg hover:bg-slate-100 hover:text-slate-900 transition-colors">
              Cancellation Policy
            </a>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-start">
          ...
          <div className="lg:col-span-8 space-y-12">
            ...
            <section id="details" className="scroll-mt-32 space-y-4">
              ...
              <div className="bg-white rounded-3xl border border-slate-200/80 shadow-2xs overflow-hidden">
                ...
                <div className="flex items-center border-b border-slate-100 bg-slate-50/50 p-1.5 overflow-x-auto no-scrollbar gap-1">
                  ...
                  <button
                    type="button"
                    onClick={() => setActiveDetailsTab("policies")}
                    className={`flex-1 min-w-[140px] py-2.5 px-4 rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-2 ${
                ...
                <div className="p-6 sm:p-8">
                  ...
                  {activeDetailsTab === "policies" && (
                    <div className="space-y-6">
                      ...
                      <div className="p-5 rounded-2xl bg-slate-50/80 border border-slate-200/80">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                          {(tour.kidsPolicy || [
                            ...
                            <div key={idx} className="p-3.5 rounded-xl bg-white border border-slate-200/60 flex items-start gap-2.5 shadow-2xs">
                              <Check className="w-4 h-4 text-teal-600 shrink-0 mt-0.5" />
                      ...
                      {/* Link to Dedicated Cancellation Section */}
                      <div className="p-4 rounded-xl bg-slate-50 border border-slate-200/70 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                        <div className="flex items-center gap-2.5 text-xs text-slate-600">
                          ...
                          <span>Official <strong>Tour Cancellation &amp; Refund Policy</strong> is featured below.</span>
                        </div>
                        <a 
                          href="#cancellation" 
                      ...
                      {/* Equipments if specified */}
                      {tour.equipments && tour.equipments.length > 0 && (
                        <div className="p-5 rounded-2xl bg-slate-50/80 border border-slate-200/80">
                          <h4 className="text-xs font-bold uppercase tracking-wider text-slate-900 mb-3 flex items-center gap-2">
                            <span className="w-2.5 h-2.5 rounded-full bg-blue-500" />
                            <span>Recommended Equipments &amp; Essentials</span>
                          </h4>
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                            {tour.equipments.map((item, idx) => (
                              <div key={idx} className="p-3 rounded-xl bg-white border border-slate-200/60 flex items-start gap-2.5">
                      ...
                      <div className="p-5 rounded-2xl bg-slate-50/80 border border-slate-200/80">
                        ...
                        <ol className="space-y-2 text-xs text-slate-600 leading-relaxed list-decimal list-inside">
                          {(tour.termsAndConditions || [
                            "The members shall not indulge in any unethical activity and may be removed from the tour without refund.",
                            "Strict zero-tolerance policy for weapons, narcotics, and illegal substances.",
                            "Vehicle AC will be operated intermittently on steep mountain climbs to prevent overheating.",
                            "Trip plan may be adjusted due to weather, roadblocks, or official safety advisories.",
                            "Organizers are not responsible for loss, theft, or damage to personal belongings.",
                            "Our trips are eco-friendly; zero waste and clean mountains are strictly observed.",
                            "Tours are uninsured; participants acknowledge inherent hazards of mountainous terrain.",
                            "Respect local cultures, traditions, and diverse group members.",
                          ]).map((item, idx) => (
            ...
            {/* E. OFFICIAL TOUR CANCELLATION & REFUND POLICY (OUTSIDE TABS) */}
            <section id="cancellation" className="scroll-mt-32 space-y-4">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div>
                  <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-[11px] font-bold tracking-wide uppercase bg-rose-50 text-rose-700 border border-rose-200/60 mb-1.5">
                  ...
                  <p className="text-xs sm:text-sm text-slate-500 mt-0.5">
                    Official refund timeline and departure regulations for all booked participants
                  </p>
                </div>
                <a
                  href="/images/cancellation-policy.png"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-1.5 px-3 py-2 rounded-xl border border-slate-200 text-slate-700 hover:text-slate-950 hover:border-slate-300 text-xs font-bold transition-all bg-white shadow-2xs self-start sm:self-auto hover:bg-slate-50"
                >
              ...
              {/* Policy Card with Table / Rows */}
              <div className="bg-white rounded-3xl border border-slate-200/80 shadow-2xs overflow-hidden p-5 sm:p-7">
                <div className="divide-y divide-slate-100">
                  {OFFICIAL_CANCELLATION_RULES.map((rule, idx) => {
                    const isFull = rule.type === "full";
                    const isHalf = rule.type === "half";

                    return (
                      <div
                        key={idx}
                        className="py-4 first:pt-0 last:pb-0 flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4 transition-colors hover:bg-slate-50/60 -mx-3 px-3 rounded-2xl"
                      >
                        <div className="flex items-center gap-3.5">
                          ...
                          >
                            {isFull && <CalendarCheck className="w-5 h-5" />}
                            {isHalf && <Calendar className="w-5 h-5" />}
                            {!isFull && !isHalf && (
                              rule.timeline.toLowerCase().includes("leave")
                          ...
                          <div>
                            <div className="text-sm font-extrabold text-slate-900 tracking-tight flex flex-wrap items-center gap-1.5">
                        ...
                        <div className="self-start sm:self-center pl-14 sm:pl-0 shrink-0">
                          <span
                            className={`inline-flex items-center px-3.5 py-1.5 rounded-xl text-xs font-black tracking-wider uppercase border shadow-2xs ${
                              isFull
                ...
                <div className="mt-6 pt-5 border-t border-slate-100">
                  <div className="p-4 rounded-2xl bg-rose-50/80 border border-rose-200/80 flex items-start gap-3">
                    ...
                    <div className="text-xs text-rose-950 leading-relaxed font-medium">
                      <strong className="font-black uppercase tracking-wider text-rose-700">Please Note: </strong>
                      {OFFICIAL_CANCELLATION_NOTE}
                    </div>
                  </div>
                </div>
              </div>
